...
task UpdateHelp {
    Import-Module .\PSFootball.psd1 -Force
...
task CopyModuleFiles {
    ...
    if (-not (Test-Path .\output\PSFootball)) {

        $null = New-Item -Path .\output\PSFootball -ItemType Directory

    }

    Copy-Item -Path '.\en-US\' -Filter *.* -Recurse -Destination .\output\PSFootball -Force
    Copy-Item -Path '.\functions\' -Filter *.* -Recurse -Destination .\output\PSFootball -Force

    #Copy Module Manifest files
    Copy-Item -Path @(
        '.\README.md'
        '.\PSFootball.psd1'
    ) -Destination .\output\PSFootball -Force
}
#endregion
...
task UpdateManifest {
    ...
    Update-ModuleManifest -ModuleVersion $newVersion -Path .\PSFootball.psd1 -ReleaseNotes $ReleaseNotes
}
#endregion
...
task PublishModule -If ($Configuration -eq 'Production') {
    ...
    if (
                ...
                $params = @{
                    Path        = ('{0}\Output\PSFootball' -f $PSScriptRoot )
                    ...
                    Write-Output -InputObject ('PSFootball module version $newVersion published to the PowerShell Gallery')
